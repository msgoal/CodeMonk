...
public:
    ...
    double findMedianSortedArraysrec(vector<int>& nums1, vector<int>& nums2) {
        ...
        while(low<=high){
            ...
            double maxLeftX = (divideX == 0 ? INT16_MIN : nums1[divideX-1]);
            double minRightX = (divideX == x ? INT16_MAX : nums1[divideX]);

            double maxLeftY = (divideY == 0 ? INT16_MIN : nums2[divideY-1]);
            double minRightY = (divideY == y ? INT16_MAX : nums2[divideY]);
